<?php
$page_title = "Collect Transport Fee";
...
if ($studentId) {
    ...
    if ($student) {
        $feeSummary = getStudentTransportFeeSummary($pdo, $studentId);
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['collect_transport_fee']) && $student) {
    $amount = (float) ($_POST['amount'] ?? 0);
    $method = trim($_POST['payment_method'] ?? 'Cash');
    $remarks = trim($_POST['remarks'] ?? '');
    $feeMonth = (int) ($_POST['fee_month'] ?? 0);

    if (!studentHasTransportAssignment($pdo, $studentId)) {
        $_SESSION['error_msg'] = 'Student has no transport route assignment.';
        header('Location: transport_fee_collect.php?student_id=' . $studentId);
        exit;
    }
    ...
    $monthStatuses = getStudentTransportMonthlyFeeStatuses($pdo, $studentId);
    ...
    if ($amount > 0) {
        ...
        $receipt = generateTransportReceiptNo($pdo);
        $insert = $pdo->prepare(
            "INSERT INTO transport_fee_payments (student_id, amount, payment_date, fee_month, payment_method, receipt_no, session_id, remarks)
             VALUES (?,?,?,?,?,?,?,?)"
        );
        $insert->execute([
            $studentId,
        ...
        $_SESSION['success_msg'] = 'Transport fee collected. Receipt: ' . $receipt;
        header('Location: transport_fee_receipt.php?id=' . $paymentId);
        exit;
    }
...
if (!$student) {
    if ($searchMode === 'quick' && $search !== '') {
        $searchResults = searchTransportStudents($pdo, $searchType, $search);
        $searchLabel = $searchType === 'name' ? 'Name: ' . $search : ($searchType === 'roll' ? 'Roll: ' . $search : 'Adm: ' . $search);
    } elseif ($searchMode === 'class' && $filterClass !== '') {
        $searchResults = getTransportStudentsByClass($pdo, $filterClass, $filterSection);
        $searchLabel = $filterClass . ($filterSection !== '' ? ' · Sec ' . $filterSection : '');
    }
}

$monthStatuses = $student && $feeSummary ? getStudentTransportMonthlyFeeStatuses($pdo, $studentId) : [];
...
$transportInfo = $feeSummary['transport'] ?? null;
?>
<div class="content-top-bar">
    <div class="content-top-main">
        <div class="content-top-icon icon-teal"><i class="fas fa-hand-holding-usd"></i></div>
        <div class="content-top-title">
            <h2>Collect Transport Fee</h2>
    ...
    <div class="content-top-actions">
        <a href="transport_fees.php" class="btn-header-action btn-header-outline"><i class="fas fa-file-invoice-dollar"></i> Transport Fee Structure</a>
    </div>
</div>
...
<div class="form-section-card section-mb fc-search-card">
    <div class="section-card-header">
        <div class="section-card-icon section-icon-transport"><i class="fas fa-search"></i></div>
        <div>
            <h4>Find Transport Student</h4>
            <p>Only students assigned to a transport route appear here</p>
        </div>
    </div>
    <div class="fc-search-tabs" role="tablist">
        <a href="transport_fee_collect.php?mode=quick" class="fc-search-tab<?php echo $searchMode === 'quick' ? ' is-active' : ''; ?>"><i class="fas fa-bolt"></i> Quick Find</a>
        <a href="transport_fee_collect.php?mode=class" class="fc-search-tab<?php echo $searchMode === 'class' ? ' is-active' : ''; ?>"><i class="fas fa-school"></i> Browse by Class</a>
    </div>
    ...
    <div class="student-search-results-head">
        <span><i class="fas fa-search"></i> <?php echo count($searchResults); ?> transport student<?php echo count($searchResults) === 1 ? '' : 's'; ?> — <?php echo htmlspecialchars($searchLabel); ?></span>
    </div>
    <?php endif; ?>
    ...
    <div class="erp-search-results student-search-results">
        ...
        <a href="transport_fee_collect.php?student_id=<?php echo (int) $r['id']; ?>" class="erp-search-item student-search-card student-search-link">
    ...
    <div class="empty-state empty-state-md">
        ...
        <h3>No transport students found</h3>
        <p>Assign a route first from Transport → Routes &amp; Assign.</p>
    </div>
    <?php endif; ?>
</div>
<?php else: ?>
...
<div class="fc-layout">
    <aside class="fc-student-panel">
        <div class="fc-hero">
            ...
            <a href="transport_fee_collect.php" class="fc-hero-btn"><i class="fas fa-search"></i> Change</a>
        </div>
        ...
        <div class="form-section-card section-mb" style="margin-top:12px">
            <div class="section-card-header">
                ...
                <div><h4>Transport</h4><p><?php echo htmlspecialchars($transportInfo['route_name']); ?><?php echo !empty($transportInfo['stop_name']) ? ' · ' . htmlspecialchars($transportInfo['stop_name']) : ''; ?><?php echo !empty($transportInfo['fare']) ? ' · ₹' . number_format((float) $transportInfo['fare'], 0) . '/mo' : ''; ?></p></div>
            </div>
        </div>
        <?php else: ?>
        <p class="fc-optional-fee-note"><i class="fas fa-info-circle"></i> No transport assignment — transport fee cannot be collected.</p>
        <?php endif; ?>
    ...
    <div class="form-section-card fc-collect-form-card">
        <div class="fc-form-head">
            <div class="fc-form-head-icon"><i class="fas fa-bus"></i></div>
            <div>
                <h4>Record Transport Payment</h4>
                <p>Separate transport receipt (TRCP) will be generated</p>
            </div>
        </div>

        <?php if (!$feeSummary['has_transport']): ?>
        <div class="empty-state empty-state-md">
            <h3>Assign transport first</h3>
            <p><a href="transport.php">Go to Transport</a></p>
        </div>
        <?php elseif ($feeSummary['fee_status'] === 'no_structure'): ?>
        <div class="empty-state empty-state-md">
            <h3>No transport fee structure</h3>
            <p>Set amounts for this class in <a href="transport_fees.php?class=<?php echo urlencode($student['class']); ?>">Transport Fee Structure</a>.</p>
        </div>
        <?php elseif (!$collectableMonths): ?>
        <div class="empty-state empty-state-md">
            <h3>All months cleared</h3>
            <p>Transport fee for this session is fully paid.</p>
        </div>
        <?php else: ?>
        <form method="POST" class="fc-collect-form">
            <input type="hidden" name="collect_transport_fee" value="1">
        ...
        <div class="table-container" style="margin-top:24px">
            <div class="table-toolbar"><strong>Transport Payment History</strong></div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><a href="transport_fee_receipt.php?id=<?php echo (int) $p['id']; ?>" target="_blank" class="fc-receipt-link"><i class="fas fa-print"></i> <?php echo htmlspecialchars($p['receipt_no']); ?></a></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
